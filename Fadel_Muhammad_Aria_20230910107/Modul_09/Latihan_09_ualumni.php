...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
    ...
    $foto_path = $row['foto']; // Default foto lama
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) { 
        ...
        if ($uploadOk == 1) {
            if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
                $foto_path = $target_file; // Perbarui path foto baru
            } else {
    ...
    $sql = "UPDATE alumni SET nama='$nama', tahun_lulus='$tahun_lulus', jurusan='$jurusan', foto='$foto_path' WHERE id=$id"; 
